...
def _ring_bounds(canvas, generation: int):
    return _descendant_ring_bounds(
...
class MultipleDescendantUnionTests(unittest.TestCase):
    ...
    def test_layout_keeps_all_union_spouses_visible(self):
        ...
        self.assertTrue(any(
            "I0893" in label and "Spouse F0335" in label
            for label in labels
        ))
        self.assertTrue(any(
            "I0893" in label and "Spouse F0340" in label
            for label in labels
        ))
        self.assertFalse(any("Spouse F0335 / Spouse F0340" in label for label in labels))
        self.assertEqual(sum("I0893" in label for label in labels), 2)
        first_generation_sectors = [
            ...
            and math.isclose(
                node.inner_radius,
                _ring_bounds(canvas, 1)[0],
            )
            and math.isclose(
                node.outer_radius,
                _ring_bounds(canvas, 1)[1],
            )
        ]
    ...
    def test_two_ring_multi_union_generation_two_cells_fit_spouses(self):
        ...
        self.assertTrue(any("Alpha" in node.content for node in texts))
        self.assertTrue(any("Beta" in node.content for node in texts))
    ...
    def test_union_cell_and_child_sector_keep_same_fill_with_empty_unions(self):
        ...
        child_ring = [
            ...
            and math.isclose(
                node.inner_radius,
                _ring_bounds(canvas, 2)[0],
            )
            and math.isclose(
                node.outer_radius,
                _ring_bounds(canvas, 2)[1],
            )
        ]
